...
while(<>) {
   ...
   if($f1 >1) { 
    ...
    #print $dd[0] ;
        $ATIME = timelocal($tt[2],$tt[1],$tt[0],$dd[2],$dd[1]-1,$dd[0]-1900) ;
   ...
   if(($f2 >1)  ) {
    ...
    @ttt=split(':',$datt[1]);
        ...
        $ATIME = timelocal($ttt[2],$ttt[1],$ttt[0],$ddd[2],$ddd[1]-1,$ddd[0]-1900) ;
  ...
  # print "$diff_oc","===","$diff_cl","\n" ;
      ...
      $Fld3 = strftime ( "%Y/%m/%d %H:%M:%S", $tm->sec, $tm->min, $tm->hour, $tm->mday, $tm->mon, $tm->year) ;
      # print "====lll====" ; 
      ...
      $Fld4 = strftime ( "%Y/%m/%d %H:%M:%S", $tm->sec, $tm->min, $tm->hour, $tm->mday, $tm->mon, $tm->year) ;
      #   print "====lll====" ;
    }
